...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Process user query and generate a response.")
    parser.add_argument("--user_id", required=True, type=str, help="User ID.")
    parser.add_argument("--input_path", type=str, help="Single file path to process.")
    parser.add_argument("--query", required=True, type=str, help="Query of the user.")
    parser.add_argument("--uuid_path", required=True, type=str, help="Output file path for the response.")
    parser.add_argument("--file_paths", type=str, help="JSON file containing multiple file paths.")
...
#             file_paths = json.load(f).get("file_paths", [])

#     # Create unique dataset paths
#     # Dataset paths
#     content_dataset_path = create_unique_dataset_path(dataset_type="content")
#     input_dataset_path = create_unique_dataset_path(dataset_type="input")

#     try:
        
#         # Process and store content dataset
#         if file_paths:
#             logger.debug("Processing multiple files into content dataset...")
#             process_and_store_files(file_paths, content_dataset_path)

#         # If input_path is provided, process it and include both datasets
#         dataset_paths = [content_dataset_path]
#         if input_path:
#             logger.debug("Processing user-provided input file into its dataset...")
#             process_and_store_files([input_path], input_dataset_path)
#             dataset_paths.append(input_dataset_path)

#         # Fetch relevant chunks from both datasets
#         relevant_chunks = fetch_relevant_chunks(query, dataset_paths)
#         reference_text = "\n".join(relevant_chunks) if relevant_chunks else "No relevant data found."

#         # Generate chatbot response
#         response = generate_chatbot_response(query, reference_text)
#         output_data = {"user_id": user_id,query: query, "answer": response,}

#         with open(uuid_path, "w") as json_file:
#             json.dump(output_data, json_file, indent=4)

#         logger.debug(f"Response successfully saved to: {uuid_path}")
...
#python chat1.py --user_id user123 --query "tell me about long rag project." --uuid_path output.json

#python chat.py --user_id user123 --input_path "C:\\Users\\Rushikesh\\Desktop\\Hridayam\\summaizer\\LongRAG Enhancing Retrieval-Augmented Generation with Long-context LLMs (1).pdf" --query "tell me about long rag project." --uuid_path output.json

#python chat.py --user_id user123 --input_path "C:\\Users\\Rushikesh\\Desktop\\Hridayam\\summaizer\\LongRAG Enhancing Retrieval-Augmented Generation with Long-context LLMs (1).pdf" --file_paths file_paths.json --query "tell me about long rag project." --uuid_path output.json
